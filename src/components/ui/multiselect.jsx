...
const MultipleSelector = ({
  ...
  // Define the value for the "Any" option
  const ANY_VALUE = "any";

  // Check if "Any" is currently selected
  const isAnySelected = React.useMemo(() => {
    return selected.some((option) => option.value === ANY_VALUE);
  }, [selected]);
                    ...
                    >
                      <>
                        {dropdowns.map((option) => {
                          // Check if "Any" is selected and this is not the "Any" option
                          const isDisabled =
                            option.disable ||
                            (isAnySelected && option.value !== ANY_VALUE);
                          ...
                          return (
                            <CommandItem
                              ...
                              onSelect={() => {
                                // If this item is disabled, do nothing
                                if (isDisabled) {
                                  return;
                                }
                                ...
                                let newOptions;
                                // If the "Any" option is selected, it's a special case
                                if (option.value === ANY_VALUE) {
                                  // If "Any" is selected, it should be the only item
                                  newOptions = [option];
                                } else if (isAnySelected) {
                                  // If another item is selected while "Any" is active,
                                  // remove "Any" and add the new item.
                                  newOptions = [option];
                                } else {
                                  // Normal multi-select behavior: add the new item
